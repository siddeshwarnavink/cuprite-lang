...
ASTDATA_CLEANUP(ast_cond_block);
...
static void _blockstack_memstk_cleanup(void *itm);
...
memstk_node *ast_create_node_data(ast_data *data, ast_node_type type) {
  ...
  case ast_logical_not:
    *data = (ast_data)malloc(sizeof(union uAstData));
    ...
    if ((*data)->expression == NULL) {
      free(*data);
      err_throw(err_fatal, "Failed to allocate memory for ast_expression_data");
    }
    break;
  default:
    *data = (ast_data)malloc(sizeof(union uAstData));
  ...
  case ast_cond_block:
    (*data)->condition_block =
        (ast_condition_block)malloc(sizeof(struct sAstConditionBlock));
    if ((*data)->condition_block == NULL) {
      free(*data);
  ...
  case ast_arithmetic_subtract:
    return memstk_push((void **)&(*data), _ast_arithmetic_subtract_cleanup);
  case ast_arithmetic_multiply:
    return memstk_push((void **)&(*data), _ast_arithmetic_multiply_cleanup);
  case ast_arithmetic_divide:
    return memstk_push((void **)&(*data), _ast_arithmetic_divide_cleanup);
  default:
    break;
  }
  return NULL;
...
void ast_parse_tokens(token_list tokens) {
  ...
  for (token_iter = tokens->tokens; token_iter != NULL;
    ...
    if (g_queue_is_empty(block_stack) &&
        (tok->type == token_eos || tok->type == token_eof)) {
      ...
      // condition block
        ...
        ast_node node = ast_parse_condition_block(line_tokens);
        if (node != NULL) {
          nodes = g_list_append(nodes, node);
        }
      }
...
ast_node ast_parse_condition_block(token_list tokens) {
  ...
  token_list expr_toks;
  token_list_create(&expr_toks);

  GList *iter = tokens->tokens->next;
  for (; iter != NULL; iter = iter->next) {
    token tok = (token)iter->data;
    if (tok->type == token_do)
      break;
    else if (tok->type == token_eos || tok->type == token_eof)
      err_throw(err_error, "Invalid use of whn");
    token tok_cpy = token_cpy(tok);
    token_list_append(&expr_toks, &tok_cpy);
  }

  ast_data node_d;
  memstk_node *node_d_memstk = ast_create_node_data(&node_d, ast_cond_block);
...
static void _extract_from_token(ast_node *node, token tok) {
  ...
  case token_num_int: {
    memstk_node *node_d_memstk = ast_create_node_data(&node_d, ast_val_int);
    node_d->val_int = atoi(tok->value->data);
    ast_create_node(node, ast_val_int, node_d);
    (*node)->data_memstk_node = node_d_memstk;
  } break;
  case token_num_float: {
    memstk_node *node_d_memstk = ast_create_node_data(&node_d, ast_val_float);
  ...
  case token_bool_t:
  case token_bool_f: {
    memstk_node *node_d_memstk = ast_create_node_data(&node_d, ast_bool);
    node_d->val_bool = tok->type == token_bool_t;
    ast_create_node(node, ast_bool, node_d);
    (*node)->data_memstk_node = node_d_memstk;
  } break;
  case token_identf: {
    memstk_node *node_d_memstk = ast_create_node_data(&node_d, ast_identf);
...
static void _make_expression_node(ast_node *node, GQueue *operator_stack,
  ...
  if (opt->type == token_not) {
    ...
    ast_data node_d;
    memstk_node *node_d_memstk = ast_create_node_data(&node_d, ast_logical_not);
    ...
    case token_plus:
      ...
      break;
    case token_hyphen:
      node_d_memstk = ast_create_node_data(&node_d, ast_arithmetic_subtract);
      break;
    case token_asterisk:
      node_d_memstk = ast_create_node_data(&node_d, ast_arithmetic_multiply);
      break;
    case token_fslash:
      node_d_memstk = ast_create_node_data(&node_d, ast_arithmetic_divide);
      break;
    case token_is:
